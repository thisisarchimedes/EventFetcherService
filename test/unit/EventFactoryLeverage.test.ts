...
describe('Leverage Events Logging & Queuing', function() {
  ...
  it('should report on PositionOpened event', async function() {
    const eventDataFileName: string = 'test/data/leveragePositionOpenedEvent.json';
    ...
    const event: OnChainEvent = await testEventGeneration(eventDataFileName, expectedEventName);
